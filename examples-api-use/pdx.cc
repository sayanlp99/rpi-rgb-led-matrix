...
static int usage(const char *progname) {
  ...
  fprintf(stderr,
          ...
          "\t-t <text>         : Enter the desired text to be displayed.\n"
          "\t-d <int>          : Enter delay in millisecond.\n"
          "\n"
          );
...
int main(int argc, char *argv[]) {
  ...
  int opt;
  while ((opt = getopt(argc, argv, "x:y:f:C:B:O:S:F:t:d:")) != -1) {
    switch (opt) {
    case 'x': x_orig = atoi(optarg); break;
    case 'y': y_orig = atoi(optarg); break;
    case 'f': bdf_font_file = strdup(optarg); break;
    case 'S': letter_spacing = atoi(optarg); break;
    case 't': display_text = strdup(optarg); printf("Entered Text: %s\n",display_text); break;
    case 'd': delay_time = atoi(optarg); printf("Delay: %d\n", delay_time); break;
    case 'C':
  ...
  char line[1024];
  char *p = unconstchar(display_text);
  strcpy(line,p);
